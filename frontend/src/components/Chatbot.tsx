...
const Chatbot = () => {
  ...
  const [messages, setMessages] = useState<Message[]>(() => {
    const saved = localStorage.getItem("influmatch_chat_history");
    return saved
      ? JSON.parse(saved)
      : [
          {
  ...
  return (
    <>
      ...
      <Dialog open={isOpen} onOpenChange={setIsOpen}>
        <DialogContent className="max-w-3xl w-full h-[85vh] p-0 overflow-hidden border-0 shadow-2xl rounded-2xl">
          {/* Header */}
          <div className="flex items-center justify-between p-4 border-b bg-gradient-to-r from-purple-600 to-orange-500 text-white">
          ...
          {/* Messages */}
          <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-gradient-to-b from-purple-50 to-orange-50">
            {messages.map((msg, i) => (
